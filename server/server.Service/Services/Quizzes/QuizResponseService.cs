    ...
    public class QuizResponseService : BaseService, IQuizResponseService
    {
        ...
        public async Task<ApiResult> SubmitQuiz(SubmitQuizModel model)
        {
            ...
            if (string.IsNullOrWhiteSpace(model.Answer))
                return ApiResult.Fail("Câu trả lời không được để trống", "VALIDATION_ERROR");
            ...
            try
            {
                ...
                if (userRoom.IsBan)
                    return ApiResult.Fail("Bạn không có quyền tham gia trả lời", "FORBIDDEN");

                // check đáp án, không phân biệt hoa thường và khoảng trắng
            ...
            {
                return ApiResult.Fail("Nộp bài thất bại", "INTERNAL_ERROR", new[] { ex.Message });
            }
        }
        ...
        public async Task<ApiResult> GetQuizStats(int quizId, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                if (userRoom == null) return ApiResult.Fail("Bạn không có quyền xem", "FORBIDDEN");
            ...
            {
                return ApiResult.Fail("Lấy thống kê thất bại", "INTERNAL_ERROR", new[] { ex.Message });
            }
        }
        ...
        public async Task<ApiResult> GetQuizSubmissions(int quizId, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                if (userRoom.Role == RoomRole.RegularUser)
                    return ApiResult.Fail("Bạn không có quyền truy cập danh sách nộp bài", "FORBIDDEN");
            ...
            {
                return ApiResult.Fail("Lấy danh sách bài nộp thất bại", "INTERNAL_ERROR", new[] { ex.Message });
            }
        }
        ...
        public async Task<ApiResult> GetRoomScoreboard(int roomId, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                if (userRoom.Role == RoomRole.RegularUser)
                    return ApiResult.Fail("Bạn không có quyền truy cập bảng điểm", "FORBIDDEN");
                ...
                var students = await (from ur in _dataContext.UserRooms.AsNoTracking()
                                      ...
                                      where ur.RoomId == roomId
                                            ...
                                            && ur.DeletedDate == null
                                      select new { u.Id, u.FullName, u.Avatar })
                                      .ToListAsync(ct);
            ...
            {
                return ApiResult.Fail("Lấy bảng điểm thất bại", "INTERNAL_ERROR", new[] { ex.Message });
            }
        }
        ...
        public async Task<ApiResult> GetMyResults(int roomId, CancellationToken ct = default)
        {
            ...
            {
                return ApiResult.Fail("Lấy kết quả bài làm thất bại", "INTERNAL_ERROR", new[] { ex.Message });
            }
        }
